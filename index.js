...
(async () => {
  ...
  data = await myFetch(`${urlFriends}`, userToken.encryptedToken);
  if (data) {
  ...
  data = await myFetch(`${urlFriends}?count=5`, userToken.encryptedToken);
  if (data) {
  ...
  let friend = await myFetch(`${urlFriends}/${data[0].friendID}`, userToken.encryptedToken);
  if (friend) {
  ...
  data = await myFetch(`${urlFriends}/${friend.friendID}`, userToken.encryptedToken, 'PUT', friend);
  if (data) {
  ...
  await myFetch(`${urlFriends}/${friend.friendID}`, userToken.encryptedToken, 'DELETE');
  
  //confirm that the friend no longer exist in the database
  data = await myFetch(`${urlFriends}/${friend.friendID}`, userToken.encryptedToken);
  if (data == null) {
  ...
  data = await myFetch(`${urlFriends}`, userToken.encryptedToken, 'POST', friend);
  if (data) {
  ...
  data = await myFetch(`${urlQuotes}`, userToken.encryptedToken);
  if (data) {
  ...
  //Test GET api/friends?count=5
  console.log("Test GET all quotes and list the 5 first");
  data = await myFetch(`${urlQuotes}?count=5`, userToken.encryptedToken);
  if (data) {
